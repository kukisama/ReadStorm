...
| `MockDownloadBookUseCase.cs` | 26 | Mock 下载 |
| `MockSearchBooksUseCase.cs` | 51 | Mock 搜索 |
...
| T-6 | 无模拟框架使用 | 低 | 未引入 `Moq` 或 `NSubstitute`。当前通过手写 Mock 实现和 `FakeHttpMessageHandler` 应对。对于当前规模可接受，但建议后续引入。 |

...
| 功能 | 状态 |
|------|------|
| 🔎 搜索 & 下载 | ✅ 完成（SearchFragment + SearchDownloadViewModel） |
| 📚 书架 | ✅ 完成（BookshelfFragment + BookshelfViewModel，2列宫格） |
| 📖 阅读器 | ✅ 完成（ReaderActivity + ReaderViewModel，沉浸式全屏） |
| 🧩 规则编辑 | ✅ 完成（RuleEditorFragment + RuleEditorViewModel） |
| 🩺 书源诊断 | ✅ 完成（DiagnosticFragment + DiagnosticViewModel） |
| ⚙️ 设置 | ✅ 完成（SettingsFragment + SettingsViewModel） |
| ℹ️ 关于 | ✅ 完成（AboutFragment + Markwon 渲染） |
| 📋 日志 | ✅ 完成（LogFragment） |
| 💾 SQLite 存储 | ✅ 完整实现（SqliteBookRepository） |
| 📁 JSON 设置 | ✅ 完整实现（JsonFileAppSettingsUseCase） |
| 📜 规则加载 | ✅ 完整实现（RuleFileLoader） |
| 📐 ViewModel 层 | ✅ 完整实现（7 个 ViewModel + 4 个辅助模型） |

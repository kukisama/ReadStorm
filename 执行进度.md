...
## 阶段进展（2026-02-16 第九次更新）

- 按用户反馈清理“测试书源 JSON 条目”影响：
  - `EmbeddedRuleCatalogUseCase` 新增测试规则过滤（template/unavailable/example-source/示例）
  - `ReadStorm.Rules/rules/rule-1.json`、`rule-2.json` 由示例规则替换为真实规则
- 书源默认选择优化：
  - 启动后默认选中“第一条真实书源”（不再默认 `全部书源`）
- 发布规则覆盖策略修正：
  - `ReadStorm.Desktop.csproj` 中规则复制改为 `CopyToOutputDirectory=Always`，避免旧 publish 文件残留导致规则错乱
- 验证结果：
  - `dotnet test tests/ReadStorm.Tests/ReadStorm.Tests.csproj` 通过（21/21）
  - `dotnet publish ... -c Release -r win-x64 --self-contained false` 成功

...
public partial class MainView : UserControl
{
    ...
    private void OnReaderVmPropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        {
            // 切章后滚动到顶部
            case "ReaderScrollVersion":
                Dispatcher.UIThread.Post(() => ReaderScrollViewer?.ScrollToHome());
                break;

            // 打开目录时隐藏工具栏
            case "IsTocOverlayVisible":
    ...
    {
        var currentTab = (DataContext as MainWindowViewModel)?.SelectedTabIndex ?? 0;
    ...
    private void UpdateNavigationUI()
    {
        ...
        var isSubPage = !_isMorePanelActive && tabIndex is TabDiagnostic or TabRules or TabSettings or TabLog;

        // 顶部栏："更多"跳板页使用主 Header，子页面和阅读器使用 SubPageHeader
        PrimaryHeader.IsVisible = !isReader && !isSubPage && (_isMorePanelActive || _activeBottomTab != 3);
        SubPageHeader.IsVisible = isReader || isSubPage;
    ...
    private void OnReaderContentTapped(object? sender, TappedEventArgs e)
    {
        ...
        var height = sv.Bounds.Height;
        if (height <= 0) return;

        var ratio = pos.Y / height;
    ...
    private void OnReaderPointerReleased(object? sender, PointerReleasedEventArgs e)
    {
        ...
        var end = e.GetPosition(sender as Visual);
        ...
        if (Math.Abs(deltaX) > 100 && Math.Abs(deltaX) > Math.Abs(deltaY) * 2)
        {
            ...
            if (deltaX > 0) // 右滑 → 上一章
            {
                if (reader.PreviousChapterCommand.CanExecute(null))
                    reader.PreviousChapterCommand.Execute(null);
            }
            else // 左滑 → 下一章
            {
                if (reader.NextChapterCommand.CanExecute(null))
                    reader.NextChapterCommand.Execute(null);
            }
        }
    }
